-- Parlay Golf Ventures: Supabase Master Schema
-- This file aggregates all current tables, enums, indexes, and RLS policies for Supabase

-- =====================
-- ENUM TYPES
-- =====================
CREATE TYPE IF NOT EXISTS contributor_type AS ENUM ('member', 'guest', 'mentor', 'creator');
CREATE TYPE IF NOT EXISTS contribution_status AS ENUM ('pending', 'approved', 'rejected');
CREATE TYPE IF NOT EXISTS member_contribution_type AS ENUM ('swing-video', 'tutorial-topic', 'personal-story');
CREATE TYPE IF NOT EXISTS guest_contribution_type AS ENUM ('swing-demo', 'idea');
CREATE TYPE IF NOT EXISTS mentor_contribution_type AS ENUM ('swing-breakdown', 'ai-tutorial');
CREATE TYPE IF NOT EXISTS content_creator_type AS ENUM ('edited-clip', 'commentary', 'gear-review', 'community-highlight');
CREATE TYPE IF NOT EXISTS swing_type AS ENUM ('driver', 'iron', 'wedge', 'putting');
CREATE TYPE IF NOT EXISTS difficulty_level AS ENUM ('beginner', 'intermediate', 'advanced');
CREATE TYPE IF NOT EXISTS analysis_type AS ENUM ('technical', 'conceptual', 'both');
CREATE TYPE IF NOT EXISTS gear_type AS ENUM ('driver', 'iron', 'wedge', 'putter', 'ball', 'accessory');
CREATE TYPE IF NOT EXISTS highlight_type AS ENUM ('achievement', 'improvement', 'community', 'other');
CREATE TYPE IF NOT EXISTS user_role AS ENUM ('member', 'mentor', 'admin');
...
CREATE POLICY "Admins can view all profiles." ON profiles FOR SELECT USING (auth.uid() IN (SELECT user_id FROM admin_users));
CREATE POLICY "Admins can update all profiles." ON profiles FOR UPDATE USING (auth.uid() IN (SELECT user_id FROM admin_users));
...
CREATE POLICY "Only admins can view admin users." ON admin_users FOR SELECT USING (auth.uid() IN (SELECT user_id FROM admin_users));
CREATE POLICY "Only admins can insert admin users." ON admin_users FOR INSERT WITH CHECK (auth.uid() IN (SELECT user_id FROM admin_users));
CREATE POLICY "Only admins can update admin users." ON admin_users FOR UPDATE USING (auth.uid() IN (SELECT user_id FROM admin_users));
CREATE POLICY "Only admins can delete admin users." ON admin_users FOR DELETE USING (auth.uid() IN (SELECT user_id FROM admin_users));
...
CREATE POLICY "Admins can view all notifications." ON content_notifications FOR SELECT USING (auth.uid() IN (SELECT user_id FROM admin_users));
...
CREATE POLICY "Admins can insert courses" ON courses FOR INSERT WITH CHECK (auth.uid() IN (SELECT user_id FROM admin_users));
CREATE POLICY "Admins can update courses" ON courses FOR UPDATE USING (auth.uid() IN (SELECT user_id FROM admin_users));
CREATE POLICY "Admins can delete courses" ON courses FOR DELETE USING (auth.uid() IN (SELECT user_id FROM admin_users));
...
CREATE POLICY "Admins can insert lessons" ON lessons FOR INSERT WITH CHECK (auth.uid() IN (SELECT user_id FROM admin_users));
CREATE POLICY "Admins can update lessons" ON lessons FOR UPDATE USING (auth.uid() IN (SELECT user_id FROM admin_users));
CREATE POLICY "Admins can delete lessons" ON lessons FOR DELETE USING (auth.uid() IN (SELECT user_id FROM admin_users));
...
CREATE POLICY "Mentors can update any contribution status" ON contributions FOR UPDATE USING (is_mentor(auth.uid()));
CREATE POLICY "Admins can perform any operation" ON contributions FOR ALL USING (EXISTS (SELECT 1 FROM auth.users WHERE id = auth.uid() AND role = 'admin'));
...
CREATE POLICY "Only service role can manage products." ON products FOR ALL USING (auth.jwt() ? auth.jwt()->>'role' = 'service_role' : false);
...
CREATE POLICY "Only service role can manage prices." ON prices FOR ALL USING (auth.jwt() ? auth.jwt()->>'role' = 'service_role' : false);
...
CREATE POLICY "Admins can view all audit logs" ON audit_logs FOR SELECT USING (EXISTS (SELECT 1 FROM auth.users WHERE id = auth.uid() AND role = 'admin'));
...
CREATE POLICY admin_all_access ON content_metrics FOR ALL TO authenticated USING (auth.jwt() ->> 'role' = 'admin');
...
CREATE POLICY admin_all_access ON platform_status FOR ALL TO authenticated USING (auth.jwt() ->> 'role' = 'admin');
...
CREATE POLICY admin_all_access ON user_signups FOR ALL TO authenticated USING (auth.jwt() ->> 'role' = 'admin');
...
CREATE POLICY "Admins can view all signup analytics" ON public.signup_analytics FOR SELECT TO authenticated USING (EXISTS (SELECT 1 FROM public.admin_users WHERE user_id = auth.uid()));
CREATE POLICY "Admins can insert signup analytics" ON public.signup_analytics FOR INSERT TO authenticated WITH CHECK (EXISTS (SELECT 1 FROM public.admin_users WHERE user_id = auth.uid()));
